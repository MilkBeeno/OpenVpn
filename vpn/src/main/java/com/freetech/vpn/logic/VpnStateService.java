...
public class VpnStateService extends Service {
    ...
    public void setUserTimeout(int timeout) {
        userTimeout = timeout * 60 * 1000;
        if (connectTime == 0) return;
        long diff = connectTime + userTimeout - System.currentTimeMillis();
        if (diff <= 0) return;
        postDisconnectMsg(diff);

    }

    private void postDisconnectMsg(long delay) {
        ...
        switch (mError) {
            case AUTH_FAILED:
                if (mImcState == ImcState.BLOCK) {
                    resId = R.string.err_assem_failed;// "Security assessment failed";
                } else {
                    resId = R.string.err_auth_failed;//return "User authentication failed";
                }
                break;
            case PEER_AUTH_FAILED:
                resId = R.string.err_peer_auth_failed;//return "Verifying server authentication failed";
                break;
            case LOOKUP_FAILED:
                resId = R.string.err_lookup_failed;//return "Server address lookup failed";
                break;
            case UNREACHABLE:
                resId = R.string.err_unreachable;//return "Server is unreachable";
                break;
            case PASSWORD_MISSING:
                resId = R.string.err_password_missing;//return "Password unavailable";
                break;
            case CERTIFICATE_UNAVAILABLE:
                resId = R.string.err_certificate_unavailable;//return "Client certificate unavailable";
                break;
            case SERVICE_UNAVAILABLE:
                resId = R.string.err_unreachable;//return "服务不可用";
                break;
            case NO_ERROR:
                resId = 0;
                break;
            default:
                resId = R.string.err_generic;//return "Unspecified failure while connecting";
        }
        if (resId > 0) {
        ...
        notifyListeners(new Callable<Boolean>() {
            ...
            public Boolean call() throws Exception {
                if (VpnStateService.this.mError != error) {
                    if (VpnStateService.this.mError == ErrorState.NO_ERROR && error != ErrorState.SERVICE_UNAVAILABLE) {
                        // setRetryTimer(error);  // 取消重连
    ...
    private long getDisconnectInterval() {
        SharedPreferences sharedPreferences = getApplicationContext().getSharedPreferences(Constants.SHARE_CFG_NAME, Context.MODE_PRIVATE);
        return sharedPreferences.getLong(Constants.SHARE_DISCONNECT_KEY, 0);//默认不断线
    }

    private boolean isNeedDisconnect() {
        long interval = getDisconnectInterval();
        //至少5分钟不断
        if (interval >= 5 * 1000 * 60 && System.currentTimeMillis() - connectTime > interval) {
            return true;
        }
        return false;
    }
